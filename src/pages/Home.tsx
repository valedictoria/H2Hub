...
const CARD_SURFACE: Record<EngineColor, string> = {
  ...
  return (
    ...
    >
      <section className="grid gap-10 pt-2 pb-4 lg:grid-cols-[1.15fr_0.85fr] lg:items-center lg:gap-12">
        <div>
          ...
          <motion.div variants={item} className="flex flex-wrap items-center gap-3">
            <a
              ...
              target="_blank"
              rel="noopener"
              className="inline-flex h-12 items-center rounded-full bg-primary px-6 text-sm font-medium text-primary-foreground transition-transform hover:-translate-y-0.5"
            >
              Watch it play on Lichess →
            </a>
            <a
              href="#engines"
              className="inline-flex h-12 items-center rounded-full border border-hairline px-6 text-sm font-medium transition-transform hover:-translate-y-0.5"
            >
              See the engines
            </a>
          </motion.div>
        ...
        <motion.div variants={item} className="rounded-3xl border border-hairline bg-card p-7">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="size-3 rounded-full bg-peach" />
      ...
      <section id="engines">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {ENGINES.map((engine) => (
            <div
              key={engine.id}
              className={`relative flex min-h-[220px] flex-col overflow-hidden rounded-3xl p-7 ${CARD_SURFACE[engine.color]}`}
            >
              <span
                aria-hidden="true"
        ...
        <div className="flex flex-wrap gap-3">
          {REPOS.map((repo) => (
            <a
              ...
              target="_blank"
              rel="noopener"
              className="rounded-full border border-hairline px-4 py-2 text-sm transition-colors hover:bg-card"
            >
              {repo.label} ↗
            </a>
          ))}
